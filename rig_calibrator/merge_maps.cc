...
void MergePoses(std::map<int, int> & cid2cid,
  ...
  for (size_t c = 0; c < blobs.size(); c++) {
    if (blobs[c].size() == 1) {
      ...
      Eigen::Quaternion<double> S = dense_map::slerp_n(W, Q);
...
void MergeMaps(dense_map::nvmData const& A,
  ...
  if (fast_merge || no_transform) {
    ...
    dense_map::KeypointVecT A_keypoint_vec, B_keypoint_vec;
    std::vector<dense_map::cameraImage> A_cams, B_cams;
    dense_map::splitTracksOneToOne(// Inputs
    ...
    for (size_t cid = 0; cid < C.cid_to_filename.size(); cid++)
      dense_map::vec2eigen(C_keypoint_vec[cid], C.cid_to_keypoint_map[cid]);
    C_keypoint_vec = dense_map::KeypointVecT (); // wipe this
  ...
  for (int cid = 0; cid < num_out_cams; cid++)
    dense_map::eigen2vec(C.cid_to_keypoint_map[cid], C_keypoint_vec[cid]);
